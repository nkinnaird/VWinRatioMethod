...
With yet another Toy MC, the resonances in the VW amplitude can be seen as a function of the time shift over the VW period, \figref{fig:JamesAnalytic_Avw_Resonances}. As shown the resonances appear at integer steps in the time shift over the VW period. 

-define numerator / ratio here in terms of delta or time shift
-say how it goes to zero at those integer steps
-show that in the function for the first plot
-compare that to the FR data plot which is nonzero
-talk about how the ratio overcompensates the amplitude in that case to account for the effect
-mention that this amplitude going large does not mean that there is a wiggle that size on the data, it's just in the ratio that the effect cancels out still 
